...
function ActionBtn({ onClick, variant = 'default', children, disabled }) {
  ...
  const variants = {
    default: `${base} bg-gray-100 text-gray-700 hover:bg-gray-200`,
    danger:  `${base} bg-red-50 text-red-600 hover:bg-red-100`,
    primary: `${base} bg-primary-50 text-primary-600 hover:bg-primary-100`,
    warn:    `${base} bg-yellow-50 text-yellow-700 hover:bg-yellow-100`,
  };
  return (
    ...
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full">
        ...
        <div className="flex gap-3">
          <button onClick={onCancel} className="flex-1 border border-gray-200 text-gray-700 py-2 rounded-xl text-sm font-medium hover:bg-gray-50 transition">Cancel</button>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-6">
      <div className="max-w-sm w-full bg-white rounded-2xl shadow-lg border border-gray-100 p-8 text-center">
        ...
        {claimError && <p className="text-red-500 text-sm mb-3">{claimError}</p>}
        <button
          onClick={onClaim}
          disabled={claiming}
          className="w-full bg-gray-900 hover:bg-gray-800 text-white py-3 rounded-xl font-semibold transition mb-3 disabled:opacity-60"
        >
          {claiming ? 'Claiming…' : 'Claim First Admin'}
        </button>
        <button onClick={onBack} className="w-full text-sm text-gray-500 hover:text-gray-700 py-2 transition">
          Go back to chats
        </button>
      </div>
    </div>
...
export default function Admin() {
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col overflow-hidden">
      ...
      {/* Header */}
      <div className="bg-gradient-to-r from-amber-700 via-orange-600 to-amber-500 text-white shadow-lg flex-shrink-0">
        <div className="max-w-6xl mx-auto flex items-center gap-3 px-4 py-4">
          {/* Desktop + mobile-nav-view: back to chats */}
          <button
            onClick={() => navigate('/')}
            className={`${mobileShowContent ? 'hidden md:flex' : 'flex'} items-center justify-center hover:bg-white/10 p-2 rounded-xl transition flex-shrink-0`}
          >
          ...
          <button
            ...
            className={`${mobileShowContent ? 'flex md:hidden' : 'hidden'} items-center justify-center hover:bg-white/10 p-2 rounded-xl transition flex-shrink-0`}
          >
            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
          ...
          {NAV.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-3 px-5 py-4 md:px-4 md:py-3 text-sm font-medium transition border-b border-gray-100 ${
                section === item.id
                  ? 'text-gray-900 md:bg-gray-900 md:text-white'
          ...
          <div className="p-4 md:p-6">
                ...
                {stats && (
                  <>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 p-5">
                      <h3 className="font-semibold text-gray-900 mb-4 text-sm uppercase tracking-wide">Quick actions</h3>
                      <div className="flex gap-3 flex-wrap">
                        <button
                          onClick={() => handleNavClick('users')}
                          className="flex items-center gap-2 px-4 py-2 bg-primary-50 text-primary-700 rounded-xl text-sm font-medium hover:bg-primary-100 transition"
                        >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-bold text-gray-900 hidden md:block">
                    Users
                    {usersTotal > 0 && <span className="ml-2 text-sm font-normal text-gray-500">({usersTotal} total)</span>}
                  </h2>
                </div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-white border border-gray-200 rounded-xl pl-9 pr-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-gray-900/10 focus:border-gray-400 transition"
                  />
                </div>
                ...
                <div className="md:hidden space-y-3">
                  ...
                  {users.map((u) => (
                    <div key={u._id} className={`bg-white rounded-2xl border border-gray-100 p-4 shadow-sm ${u.isBanned ? 'opacity-60' : ''}`}>
                ...
                <div className="hidden md:block bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-50">
                      ...
                      {users.map((u) => (
                        <tr key={u._id} className={`hover:bg-gray-50 transition ${u.isBanned ? 'opacity-60' : ''}`}>
                          ...
                          <td className="px-4 py-3">
                            <span className={`w-2 h-2 rounded-full inline-block mr-1.5 ${u.isOnline ? 'bg-green-500' : 'bg-gray-300'}`} />
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="px-4 py-2 text-sm bg-white border border-gray-200 rounded-xl hover:bg-gray-50 transition disabled:opacity-40"
                      >
                        Previous
                      </button>
                      <button
            ...
            {section === 'system' && (
              <div className="max-w-lg space-y-5">
                <h2 className="text-xl font-bold text-gray-900 hidden md:block">System</h2>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 divide-y divide-gray-100 shadow-sm">
                  ...
                  <div className="px-5 py-4 flex justify-between items-center">
                    <span className="text-sm text-gray-600 font-medium">WebSocket</span>
                    <span className="flex items-center gap-1.5 text-sm text-green-600 font-semibold">
                      <span className="w-2 h-2 bg-green-500 rounded-full" /> Active
                    </span>
                  </div>
                  <div className="px-5 py-4 flex justify-between items-center">
                    <span className="text-sm text-gray-600 font-medium">Encryption</span>
                    <span className="text-sm text-blue-600 font-semibold">RSA-OAEP + AES-GCM</span>
                  </div>
                  <div className="px-5 py-4 flex justify-between items-center">
                    <span className="text-sm text-gray-600 font-medium">Message cleanup</span>
                    <span className="text-sm text-gray-500">Every 60 seconds</span>
                  </div>
                  <div className="px-5 py-4 flex justify-between items-center">
                    <span className="text-sm text-gray-600 font-medium">Media storage</span>
                    <span className="text-sm text-gray-500">Local / uploads/</span>
                  </div>
                </div>
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-2xl px-5 py-4">
                  ...
                  <p className="text-xs text-yellow-700 leading-relaxed">
                    To manually grant admin to a user, set <code className="bg-yellow-100 px-1 rounded">isAdmin: true</code> in MongoDB, or use <strong>Claim First Admin</strong> at <code>/admin</code> when no admins exist.
                  </p>
                </div>
              </div>
            )}
