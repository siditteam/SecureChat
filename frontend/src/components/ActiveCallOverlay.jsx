...
function CtrlBtn({ onClick, active, danger, title, children }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      <span style={{ color: 'var(--text-secondary)', fontSize: 10, fontWeight: 500 }}>{title}</span>
    </div>
  );
}
    ...
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-between bg-gradient-to-b from-ink-900 via-ink-800 to-ink-900 py-16 px-8">
        ...
        <div className="text-center">
          <p style={{ color: 'var(--text-secondary)', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.12em' }}>
            {isOutgoing ? 'Calling…' : 'On Call'}
          </p>
          {isActive && <p style={{ color: 'var(--accent)', fontSize: 14, marginTop: 6, fontFamily: 'monospace' }}>{formatDuration(callDuration)}</p>}
          {callError && <p style={{ color: '#ef4444', fontSize: 14, marginTop: 6 }}>{callError}</p>}
        </div>
        ...
        <div className="flex flex-col items-center gap-6">
          ...
          <div className="text-center">
            <p style={{ color: 'var(--text-primary)', fontSize: 20, fontWeight: 700 }}>{remoteUser?.username}</p>
            {isActive && (
        ...
        <div className="flex items-center justify-center gap-8">
          <CtrlBtn onClick={toggleMute} active={isMuted} title={isMuted ? 'Unmute' : 'Mute'}>
            {isMuted ? (
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" style={{ color: 'var(--text-on-accent, #fff)' }}>
          ...
          <CtrlBtn onClick={endCall} danger title="End">
            <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24" style={{ color: 'var(--text-on-accent, #fff)' }}>
  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 left-0 right-0 z-10 px-5 pt-5 pb-8 bg-gradient-to-b from-black/70 to-transparent transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
        <p style={{ color: 'var(--text-primary)', fontWeight: 700, fontSize: 18 }}>{remoteUser?.username}</p>
        {isActive && <p style={{ color: 'var(--accent-strong)', fontSize: 12, fontFamily: 'monospace', marginTop: 4 }}>{formatDuration(callDuration)}</p>}
        {callError && <p style={{ color: '#ef4444', marginTop: 6 }}>{callError}</p>}
      </div>
